...
class NeuralManager:
    def __init__(self, loglikelihood, pars_bounds, rootname, ndivsgrid=50, hidden_layers_neurons=None,
                 epochs=100, **kwargs):
        ...
        if not self.modelChecker():
            self.neural_model = self.training()
        else:
            self.neural_model = self.load()

    def training(self):
        grid = GridLikes(self.loglikelihood, self.pars_bounds, ndivs=self.ndivsgrid)
